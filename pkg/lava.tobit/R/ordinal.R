...
`ordinal.lvm` <- function(x,var=NULL,K=2, constrain, start, ...) {
  if (is.null(var)) {
    ...
    ii <- x$attributes$iordinal
    fix <-   lapply(ii,function(i) x$exfix[i])
    if (length(ordidx)>0) {
      val <- names(ordidx)
      return(structure(val,K=KK,idx=ii,fix=fix,class="ordinal.lvm"))
    }
    else
      return(NULL)
  }
  ...
  for (i in seq_len(length(var))) {
    if (K[i]>2) {
      ...
      idx <- length(x$expar)+seq(length(newpar)); names(idx) <- names(newpar)
      if (any(pex <- parname%in%names(x$expar))) {
        if (!all(pex))
          stop("Cannot change number of categories! Re-specify model.")
        x$exfix[parname] <- newfix
      } else {
