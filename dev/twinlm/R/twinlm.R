...
twinlm <- function(formula, data, id, zyg, DZ, DZos, weight=NULL, type=c("ace"), twinnum="twinnum", binary=FALSE,probitscale=1,keep=weight,estimator="gaussian",control=list(method="NR"),...) {
  ## if (binary) {
...
##  parameter(model2) <- ~sdu1+sdu2
  ...
  if (binary) {
    e <- estimate(mg,weight2=weight,estimator=estimator,fix=FALSE,control=control,...)
  } else {
    e <- estimate(mg,weight=weight,estimator=estimator,fix=FALSE,control=control,...)
  }
